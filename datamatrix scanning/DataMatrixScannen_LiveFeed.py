...
def data_matrix_demo(cap):
    ...
    while True:
        ...
        imdecode = imdecode[top:bottom, left:right]

        # Start decode process every 0.1s
        ...
        cv2.imshow(window_name, imdecode)
        key = cv2.waitKey(1)
...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        ...
        cap = cv2.VideoCapture(0) # externe c``amera
    else:
        print("FUck")
    ...
    cap.set(cv2.CAP_PROP_FOCUS, 175) 
